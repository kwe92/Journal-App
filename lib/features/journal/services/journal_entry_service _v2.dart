...
typedef JournalEntries = List<JournalEntryV2>;

/// handles all C.R.U.D API calls for journal entries based on the currently authenticated and logged in user.
class JournalEntryServiceV2 extends ApiService with ListenableServiceMixin {
  ...
  Future<void> addEntry(JournalEntryV2 newEntry) async {
  ...
  Future<void> updateEntry(JournalEntryV2 updatedEntry) async {
  ...
  Future<void> deleteEntry(JournalEntryV2 entry) async {
  ...
  List<JournalEntryV2> sortByUpdatedDate(List<Map<String, dynamic>> entries, [bool asc = false]) {
    if (asc) {
      return entries.map((entry) => JournalEntryV2.fromJSON(entry)).toList().sorted(
  ...
  DateTime getMaxDate(List<JournalEntryV2> entries) => entries.reduce((a, b) => a.createdAt.isAfter(b.createdAt) ? a : b).createdAt;

  DateTime? getMinDates(List<JournalEntryV2> entries) =>
